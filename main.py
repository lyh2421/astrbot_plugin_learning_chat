...
class LearningChatPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        # 保存 AstrBotConfig 引用，用于后续动态读取 total_enable
        self._astrbot_cfg = config
    ...
    def _is_total_enabled(self) -> bool:
        """动态检查 total_enable，兼容 WebUI 禁用/启用切换"""
        if self._astrbot_cfg:
            try:
                return self._astrbot_cfg.get("total_enable", True)
            except Exception:
                pass
        return self.config.total_enable

    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE)
    async def on_group_message(self, event: AstrMessageEvent):
        """处理群消息，执行学习+回复逻辑"""
        # 动态检查 WebUI 总开关 (修复：禁用后仍生效)
        if not self._is_total_enabled():
            return
